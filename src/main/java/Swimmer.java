...
public class Swimmer implements Comparable<Swimmer> {
    ...
    public int compareTo(Swimmer otherSwimmer) {
        ...
        double thisAverage = thisSum / this.freestyle50mTimes.size();
        double otherSum = 0.0;

        for (Double time : otherSwimmer.getFreestyle50mTimes()){
            otherSum += time;
        }

        double otherAverage = otherSum / otherSwimmer.getFreestyle50mTimes().size();
